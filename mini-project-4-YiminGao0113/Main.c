...
unsigned long ConsumerTaskDone;
unsigned long ConsumerWaitingforFIFO;
unsigned long ConsumerStart;
unsigned long ConsumerTime;
...
void ButtonWork(void){
	...
	ElapsedTime = 0;
	OS_bWait(&LCDFree);
	Button1RespTime = OS_MsTime() - Button1PushTime;
...
void Consumer(void){
	while(NumSamples < RUNLENGTH){
		jsDataType data;
		ConsumerTaskDone = OS_MsTime();
		JsFifo_Get(&data);
		ConsumerWaitingforFIFO = OS_MsTime() - ConsumerTaskDone;
		OS_bWait(&LCDFree);
		ConsumerStart = OS_MsTime();
		...
		ConsumerCount++;
		ConsumerTime = OS_MsTime() - ConsumerStart;
		OS_bSignal(&LCDFree);
		prevx = data.x; 
		prevy = data.y;
	  // OS_Suspend();
...
void Restart(void){
	...
	ElapsedTime = 0;
	OS_bWait(&LCDFree);
	Button2RespTime = OS_MsTime() - Button2PushTime;
	BSP_LCD_FillScreen(BGCOLOR);
