...
void BSP_LCD_Message (int device, int line, int col, char *string, unsigned int value){
  ...
  length = BSP_LCD_DrawString(col, line, string, LCD_WHITE);
  BSP_LCD_SetCursor(col + length, line);
  BSP_LCD_OutUDec(value, LCD_WHITE);

  if (value < 100 && value > 9)  // If the value only has 2 digits, set the third digit black
    BSP_LCD_DrawString(col + length + 2, line, 0, LCD_BLACK);
  else if (value < 10) // If the value only has 1 digit, set the second and third digits black
    BSP_LCD_DrawString(col + length + 1, line, "00", LCD_BLACK);
...
void BSP_LCD_DrawCrosshair(int16_t x, int16_t y, int16_t color) {
  // Your Code Here
  BSP_LCD_DrawFastVLine(x, (y-2), 5, color);
