...
void OS_Signal(Sema4Type *semaPt){
	#ifdef blockSema
		tcbType * temp_pt = RunPt->next;
		OS_DisableInterrupts();
		semaPt->Value += 1;
		if (semaPt->Value <= 0){
			while(temp_pt -> blockPt != semaPt) {
...
void Scheduler(void){
			...
			pt = RunPt;
					...
					if((pt-> WorkPriority < max)&&((pt-> blockPt) == 0)&&((pt-> sleepCt) == 0)){
							...
							// find highest priority thread not blocked or sleeping
							do{
							if((pt-> priority < max)&&((pt-> blockPt) == 0)&&((pt-> sleepCt) == 0)){
								max = pt-> priority; bestPt = pt;}
								pt = pt-> next;
								} while( RunPt != pt);
							// look at all possible threads
							RunPt = bestPt;
					#endif
		#else  //RoundRobin Blocking


			RunPt = RunPt->next;
			while (RunPt->blockPt || RunPt->sleepCt){
				RunPt = RunPt->next;
			}
		#endif
	...
	MSTime++;
			...
			for(i = 0; i < NUMTHREADS; i++) {
				if(!tcbs[i].available){
							...
							if((RunPt!=&tcbs[i]) && !(tcbs[i].blockPt)){
								tcbs[i].age += 1;
							}
